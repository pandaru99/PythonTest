...
1. **콤보박스로 날짜 선택**  
   - 최근 60일 달력 날짜가 콤보박스에 표시됩니다.

2. **선택된 일자 → 직전 영업일 자동 적용**  
   - 날짜를 고르면 `to_business_day()`(= `get_nearest_business_day_in_a_week`)로 **항상 직전 영업일**로 보정 후 조회합니다.  
   - 주말/공휴일을 골라도 문제없이 직전 영업일로 조회됩니다.

3. **다른 ETF 선택 시 현재 조회조건(날짜)으로 조회**  
   - ETF를 바꾸면 **현재 날짜 콤보박스에 선택된 값**을 기준으로 직전 영업일을 적용해서 바로 재조회합니다.

이제 날짜와 ETF를 자유롭게 바꿔가며 조회할 수 있습니다.