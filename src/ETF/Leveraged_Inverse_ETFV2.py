...
REAL_FUNCTION = {
    # 인버스 2X
    "000001": "get_etf_portfolio_deposit_file",
    "000002": "get_market_ohlcv",
    
    # 레버리지 2X (삼성전자)
    "000003": "get_etf_ohlcv_by_date",
    "000004": "get_etf_portfolio_deposit_file",
    "000005": "get_market_ohlcv",
    "000007": "KIWOOM 삼성전자선물단일종목레버리지",
    "000008": "1Q 삼성전자선물단일종목레버리지",
    "000009": "ACE 삼성전자단일종목레버리지",
    
    # 레버리지 2X (SK하이닉스)
    "000010": "KODEX SK하이닉스단일종목레버리지",
    "000011": "TIGER SK하이닉스단일종목레버리지",
    "000012": "SOL SK하이닉스단일종목레버리지",
    "000013": "RISE SK하이닉스단일종목레버리지",
    "000014": "KIWOOM SK하이닉스선물단일종목레버리지",
    "000015": "ACE SK하이닉스단일종목레버리지",
    "000016": "1Q SK하이닉스선물단일종목레버리지",
}
...
def get_portfolio_df(selected_ticker: str, selected_date: str, selected_func: str) -> tuple:
    ...
    try:
        ...
        match status:
            ...
            case "000002":
                df = stock.get_market_ohlcv('20260701', safe_date, selected_ticker)
                if df is None or df.empty:
                    df = stock.get_market_ohlcv(selected_ticker)
            case "000003":
                pdf = stock.get_etf_ohlcv_by_date('20260701', safe_date, selected_ticker)
                if df is None or df.empty:
                    df = stock.get_etf_ohlcv_by_date(selected_ticker)
    ...
    def on_etf_select(event=None):
        ...
        if selected:
            ...
            if new_ticker != current["ticker"]:
                ...
                reload_data(new_ticker=new_ticker, selected_calendar_date=combo_date.get(), new_func=new_func)
